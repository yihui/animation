...
#' A Wrapper for the `convert' Utility of ImageMagick
#' 
#' This function simply wraps the arguments of the \command{convert} utility of
#' ImageMagick to make it easier to call \command{convert} in R.
#' 
...
#'   animation for an infinite number of times, otherwise the animation repeats
#'   itself up to \code{loop} times (N.B. for GIF only!)
#' @param output the file name of the output (with proper extensions, e.g. gif)
...
#' @param cmd.fun a function to invoke the OS command; by default, \code{shell}
#'   under Windows and \code{\link{system}} under other OS
...
#'   \url{http://www.imagemagick.org} for details), and \command{convert} is in
#'   your \code{'PATH'} variable, in which case the command \command{convert}
#'   can be called without the full path (otherwise you have to set
#'   \command{convert} to be, e.g., \file{C:/Programe
#'   Files/ImageMagick/convert.exe}). Windows users are often very confused
#'   about the ImageMagick and \code{'PATH'} setting, so I'll try to search for
