...
#' \ldotsx1, x2, \dots}; tangent lines are drawn to illustrate the relationship
#' between successive iterations; the iteration values are in the right margin
#' of the plot.
#' 
...
#'   \item{value the value of \code{FUN(root)}} \item{iternumber of iterations;
#'   if it is equal to \code{control$nmax}, it's quite likely that the root is
#'   not reliable because the maximum number of iterations has been reached}
