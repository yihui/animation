...
#' GIF/MPG animations or PDF animations are also available if required software
...
#'   to the site during last \emph{60 min}.} \item{\code{pages}number of times
#'   a \emph{page} of the site is viewed (sum for all visitors for all visits).
#'   This piece of data differs from ``files'' in that it counts only HTML
#'   pages and excludes images and other files.} \item{\code{files}number of
#'   times a \emph{page, image, file} of the site is viewed or downloaded by
#'   someone.} \item{\code{bandwidth}amount of data downloaded by all
#'   \emph{pages}, \emph{images} and \emph{files} within the site (units in
#'   MegaBytes).} }
...
#' 3848 observations on 5 variables. From the 1986 ASA Data Exposition dataset,
#' made up by David Coleman of RCA Labs.
#' 
#' 
#' @name pollen
#' @docType data
