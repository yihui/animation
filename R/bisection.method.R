##' Demonstration of the Bisection Method for root-finding on an
##' interval
##'
##' This is a visual demonstration of finding the root of an equation
##' \eqn{f(x) = 0} on an interval using the Bisection Method.
##'
##' Suppose we want to solve the equation \eqn{f(x) = 0}. Given two
##' points a and b such that \eqn{f(a)} and \eqn{f(b)} have opposite
##' signs, we know by the intermediate value theorem that \eqn{f} must
##' have at least one root in the interval \eqn{[a, b]} as long as
##' \eqn{f} is continuous on this interval. The bisection method
##' divides the interval in two by computing \eqn{c = (a + b) /
##' 2}. There are now two possibilities: either \eqn{f(a)} and
##' \eqn{f(c)} have opposite signs, or \eqn{f(c)} and \eqn{f(b)} have
##' opposite signs. The bisection algorithm is then applied
...
##' annotated in the graph by both texts and blue straight lines, and
##' the end-points are denoted in dashed red lines. The root of each
##' iteration is also plotted in the right margin of the graph.
##'
##' @param FUN the function in the equation to solve (univariate)
##' @param rg a vector containing the end-points of the interval to be
##' searched for the root; in a \code{c(a, b)} form
##' @param tol the desired accuracy (convergence tolerance)
##' @param interact logical; whether choose the end-points by cliking
##' on the curve (for two times) directly?
