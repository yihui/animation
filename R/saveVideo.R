...
##' more details on the numerous options of FFmpeg, please see the
##' reference.
##' @param expr the R code to draw (several) plots
##' @param img.name the file name of the sequence of images to be
##' generated
##' @param video.name the file name of the output video
##' (e.g. \file{animation.mp4} or \file{animation.avi})
##' @param ffmpeg the command to call FFmpeg
##' (e.g. \code{"C:/Software/ffmpeg/bin/ffmpeg.exe"} under Windows);
##' note the full path of FFmpeg can be pre-specified in
##' \code{\link{ani.options}('ffmpeg')}
##' @param other.opts other options to be passed to \code{ffmpeg},
##' e.g. we can specify the bitrate as \code{other.opts = "-b 400k"}
##' @param clean whether to remove the sequence of images after
##' conversion
...
##' @author Yihui Xie <\url{http://yihui.name}>, based on an inital
##' version by Thomas Julou \email{thomas.julou@@gmail.com}
##' @note There are a lot of possibilities in optimizing the video. My
##' knowledge on FFmpeg is very limited, hence the default output by
##' this function could be of low quality or too large. The file
##' \file{presets.xml} of WinFF might be a good guide:
