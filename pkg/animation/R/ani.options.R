ani.options <- function(...) {
    mf = list(interval = 1, nmax = 50, ani.width = 480, ani.height = 480, 
        outdir = tempdir(), filename = "index.htm", withprompt = "ANI> ", 
        ani.type = "png", ani.dev = png, title = "Statistical Animations Using R", 
        description = "This is an animation.", footer = TRUE, 
        loop = TRUE, autobrowse = TRUE)
    ...
    if (length(lst)) {
        if (is.null(names(lst)) & !is.list(lst[[1]])) {
            if (identical(unlist(lst), "interval") & !interactive()) 
                return(0)
            else return(getOption("ani")[unlist(lst)][[1]])
