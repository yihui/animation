...
\begin{enumerate}
\item Stepwise -- By $C_{p},AIC,BIC,p-value$
\item Press-xie sir 10th HW
\end{enumerate}
...
larger and larger, the restriction on $\pi$ becomes less important).\label{fig:binom-norm-approx}}
...
# n * pi and n * (1 - pi)
np=c(outer(1:100, seq(0,1,.005), '*')) 
n1p=c(outer(1:100, 1-seq(0,1,.005), '*')) 
# condition for good approximation
cond=(np>=5) & (n1p>=5) 
# plot with semi-transparent colors: green if cond == TRUE, red else
plot(np,c(pval),pch=20,col=ifelse(cond,rgb(0,1,0,.2),rgb(1,0,0,.2)), xlab=expression('n *'~pi), ylab = 'P-value') 
@
