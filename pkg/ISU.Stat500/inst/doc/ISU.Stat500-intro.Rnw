...
and output lines are using \texttt{normal monospaced font}. R package
names are in \textbf{bold font}, and function names in \textit{italics}.
...
\subsubsection{Multiple comparison procedures}

LSD, Scheffe, Bonferroni, Tukey-Kramer


\subsubsection{Adjusting p-values}

There are several methods for adjusting p-values provided in the function
\fun{p.ajust()} (in \pkg{stats}), whose names are stored in \texttt{p.adjust.methods}:

<<>>=
p.adjust.methods 
@

Benjamini \& Hochberg's method for controlling the False Discovery
Rate (FDR) is mentioned in this course; adjusted p-values can be directly
computed as \texttt{p {*} length(p) / rank(p)} where \texttt{p} is
the vector of original p-values%
\footnote{this simple computation might be problematic as the result p-values
can be larger than 1, so we recommend using \fun{p.ajust()}; you
may read its source code to learn how to do {}``safe'' programming.%
}), or we may use \fun{p.ajust()}:
...
SSE becomes smaller. See \texttt{?least.squares} in \pkg{animation}
\citep{xie09}.\label{fig:OLS-slope}}
